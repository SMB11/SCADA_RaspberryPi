...
stop_blowing_for_filling_stopped = tk.BooleanVar()  # New variable
...
tk.Checkbutton(auto_tab, text="Stop Blowing if Filling Stopped", variable=stop_blowing_for_filling_stopped).pack(anchor="w", padx=5, pady=2)  # New checkbox
...
blowing_stop_delay_value = tk.IntVar(value=5)  # New variable
...
tk.Label(settings_tab, text="Blowing Stop Delay (s):").pack(anchor="w", padx=5, pady=2)
tk.Entry(settings_tab, textvariable=blowing_stop_delay_value).pack(fill="x", padx=5, pady=2)
tk.Button(settings_tab, text="Update Blowing Stop Delay", command=update_blowing_stop_delay).pack(pady=5)
