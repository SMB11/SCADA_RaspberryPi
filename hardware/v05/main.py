...
canvas = ctk.CTkCanvas(root, width=700, height=700, bg="#2b2b2b", highlightthickness=0)
...
sensor1_label = ctk.CTkLabel(status_frame, text=f"Sensor1 Counter: {sensor1_counter}", font=("Arial", 12))
sensor1_label.pack(pady=5)
sensor2_label = ctk.CTkLabel(status_frame, text=f"Sensor2 Counter: {sensor2_counter}", font=("Arial", 12))
sensor2_label.pack(pady=5)
sensor1_traffic_label = ctk.CTkLabel(status_frame, text="Sensor1 Traffic: Clear", font=("Arial", 12))
sensor1_traffic_label.pack(pady=5)
sensor2_traffic_label = ctk.CTkLabel(status_frame, text="Sensor2 Traffic: Clear", font=("Arial", 12))
sensor2_traffic_label.pack(pady=5)

# Tab control setup
tab_control = ctk.CTkNotebook(root)
manual_tab = ctk.CTkFrame(tab_control)
auto_tab = ctk.CTkFrame(tab_control)
settings_tab = ctk.CTkFrame(tab_control)
status_tab = ctk.CTkFrame(tab_control)
...
for text, cmd in [("Start Labeling", start_labeling_machine), ("Stop Labeling", stop_labeling_machine),
    ...
    button = ctk.CTkButton(manual_tab, text=text, command=cmd)
    button.pack(padx=5, pady=5, fill='x')

# Automatic Control Tab
stop_filling_for_traffic = ctk.BooleanVar()
stop_labeling_for_traffic = ctk.BooleanVar()
stop_labeling_for_timeout = ctk.BooleanVar()
stop_blowing_for_filling_stopped = ctk.BooleanVar()  # New variable

ctk.CTkCheckBox(auto_tab, text="Stop Filling for Sensor1 Traffic", variable=stop_filling_for_traffic).pack(anchor="w", padx=5, pady=5)
ctk.CTkCheckBox(auto_tab, text="Stop Labeling for Sensor2 Traffic", variable=stop_labeling_for_traffic).pack(anchor="w", padx=5, pady=5)
ctk.CTkCheckBox(auto_tab, text="Stop Labeling if Sensor1 Inactive", variable=stop_labeling_for_timeout).pack(anchor="w", padx=5, pady=5)
ctk.CTkCheckBox(auto_tab, text="Stop Blowing if Filling Stopped", variable=stop_blowing_for_filling_stopped).pack(anchor="w", padx=5, pady=5)
ctk.CTkButton(auto_tab, text="Toggle Auto Mode", command=toggle_auto_mode).pack(padx=5, pady=10, fill='x')

# Auto Mode Indicator
auto_mode_indicator = ctk.CTkLabel(root, text="Manual Mode Enabled", font=("Arial", 14))
auto_mode_indicator.grid(row=0, column=1, padx=5, pady=5, sticky="s")

# Settings Tab
labeling_timeout_value = ctk.StringVar(value='5')
traffic_threshold_value_sensor1 = ctk.StringVar(value='2')
traffic_threshold_value_sensor2 = ctk.StringVar(value='2')
blowing_stop_delay_value = ctk.StringVar(value='5')  # New variable
...
ctk.CTkLabel(settings_tab, text="Labeling Timeout (s):").pack(anchor="w", padx=5, pady=5)
ctk.CTkEntry(settings_tab, textvariable=labeling_timeout_value).pack(fill="x", padx=5, pady=5)
ctk.CTkButton(settings_tab, text="Update Timeout", command=update_labeling_timeout).pack(pady=5, padx=5, fill='x')

ctk.CTkLabel(settings_tab, text="Traffic Threshold Sensor1 (s):").pack(anchor="w", padx=5, pady=5)
ctk.CTkEntry(settings_tab, textvariable=traffic_threshold_value_sensor1).pack(fill="x", padx=5, pady=5)
ctk.CTkButton(settings_tab, text="Update Threshold Sensor1", command=update_traffic_threshold_sensor1).pack(pady=5, padx=5, fill='x')

ctk.CTkLabel(settings_tab, text="Traffic Threshold Sensor2 (s):").pack(anchor="w", padx=5, pady=5)
ctk.CTkEntry(settings_tab, textvariable=traffic_threshold_value_sensor2).pack(fill="x", padx=5, pady=5)
ctk.CTkButton(settings_tab, text="Update Threshold Sensor2", command=update_traffic_threshold_sensor2).pack(pady=5, padx=5, fill='x')

ctk.CTkLabel(settings_tab, text="Blowing Stop Delay (s):").pack(anchor="w", padx=5, pady=5)
ctk.CTkEntry(settings_tab, textvariable=blowing_stop_delay_value).pack(fill="x", padx=5, pady=5)
ctk.CTkButton(settings_tab, text="Update Blowing Stop Delay", command=update_blowing_stop_delay).pack(pady=5, padx=5, fill='x')

# System Status Tab for machine statuses
blowing_status_label = ctk.CTkLabel(status_tab, text="Blowing Working: Inactive", font=("Arial", 12))
blowing_status_label.pack(pady=5)
filling_status_label = ctk.CTkLabel(status_tab, text="Filling Working: Inactive", font=("Arial", 12))
filling_status_label.pack(pady=5)
labeling_status_label = ctk.CTkLabel(status_tab, text="Labeling Working: Inactive", font=("Arial", 12))
labeling_status_label.pack(pady=5)

blowing_alarm_label = ctk.CTkLabel(status_tab, text="Blowing Alarm: Inactive", font=("Arial", 12))
blowing_alarm_label.pack(pady=5)
filling_alarm_label = ctk.CTkLabel(status_tab, text="Filling Alarm: Inactive", font=("Arial", 12))
filling_alarm_label.pack(pady=5)
labeling_alarm_label = ctk.CTkLabel(status_tab, text="Labeling Alarm: Inactive", font=("Arial", 12))
labeling_alarm_label.pack(pady=5)

labeling_idle_label = ctk.CTkLabel(status_tab, text="Labeling Idle: Inactive", font=("Arial", 12))
labeling_idle_label.pack(pady=5)
filling_idle_label = ctk.CTkLabel(status_tab, text="Filling Idle: Inactive", font=("Arial", 12))
